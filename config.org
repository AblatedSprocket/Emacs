...
#+BEGIN_SRC emacs-lisp :tangle yes :padline no
  ...
  (straight-use-package '(everforest-theme-emacs :host github :repo "ablatedsprocket/everforest-theme-emacs"))
...
(require 'gruvbox)
(if (daemonp)
    (load-theme 'everforest-dark-medium t)
...
Download the files from Langtool's [[https://dev.languagetool.org/http-server.html][website]].
...
(defun apply-post-frame-org-config ()
  (set-face-attribute 'org-block nil :background (face-background 'org-block-begin-line) :inherit 'fixed-pitch)
