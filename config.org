...
#+END_SRC
*** ACTIVE Ripgrep
I use Ripgre (rg) for searching through files for text when a language-server can't do the job.
#+BEGIN_SRC emacs-lisp
(require-package 'rg)
#+END_SRC
...
If you don't want this functionality, simply set the state from =ACTIVE= to =DISABLED=. If you do want it, there are a few things to install to make Emacs work as a mail client. The first task is to install Mu4e dependencies: =sudo apt install mu4e isync html2text gnupg2=.
...
I have a lot of customization for Mmu4e. Admittedly, most of it was taken from other peoples' configuration I found online. An interesting aspect of Mu4e contexts, which can be associated with an email address. This provides separation between work and home, for example.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Prose
I like using Emacs as a code editor and for creative writing. Emacs doesn't have a lot of the things a standard word processor does (especially by default), but I've found the packages that provide a better experience for me than Word or any other word processor.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
***** ACTIVE Blacken
Currently, I use Blacken for Python code formatting. Assuming you have installed the required packages for [[* Python][Python]], you will need to install Black:
#+BEGIN_SRC shell
pip3 install black
#+END_SRC
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
***** ACTIVE LSP-Pyls
Python language server provides the backend for LSP-mode. The one thing I dislike about Python and pep8 in general is the "line too long" suggestions. Assuming you have installed the packages required for [[* Python][Python]], you need to install PyLS:
#+BEGIN_SRC shell
...
#+END_SRC
***** ACTIVE Pyvenv
I use Pyvenv to interface with my projects' virtual environments. Assuming you have installed the packages in [[* Python][Python]], You will need to install Venv:
#+BEGIN_SRC shell
