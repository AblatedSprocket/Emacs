...
(defcustom ejira-org-todo-keywords-alist nil
  "List of Org todo keywords to be inserted into Ejira org file so org-todo-keywords do not need to be set globally."
  :group 'ejira
  :tag "JIRA state map"
  :type '(alist :key-type (string :tag "JIRA Project ID")
                :value-type ('(repeat string) :tag "org keywords")))
  ...
  (save-window-excursion
    (save-current-buffer
      (let* ((existing-heading (ejira--find-heading key))
        ...
        (unless exists-p
          (if (assoc key ejira-org-todo-keywords-alist)
              (let ((todos ""))
                (dolist (item (cdr (assoc key ejira-org-todo-keywords-alist)))
                  (setq todos (concat todos " " item)))
                (write-region (format "#+STARTUP: showeverything\n#+TODO:%s\n" todos) nil project-file-name))
